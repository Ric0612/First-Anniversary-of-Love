...
MiniStar.prototype.draw = function() {
    ...
    c.beginPath()
    c.arc(this.x, this.y, this.radius, 0, Math.PI * 2, false)
    c.fillStyle = `rgba(255,182,193, ${this.opacity})`
    c.shadowColor = '#ffb0c8'
    c.shadowBlur = 16
    c.fill()
    c.closePath()
    c.restore()
...
backgroundGradient.addColorStop(0, '#2b1020')   
backgroundGradient.addColorStop(1, '#ffd1e8')   
...
function init() {
    ...
    for(let i = 0; i < 200; i++){
        ...
        backgroundStars.push(new Star(x, y, radius, 'rgba(255,200,220,0.9)'))
    }
}
...
function animate() {
    ...
    if(flag) creatMountainRange(1, canvas.height * 0.7, '#4a1020')
    if(flag) creatMountainRange(2, canvas.height * 0.6, '#3a0f1c')
    if(flag) creatMountainRange(3, canvas.height * 0.4, '#2b0812')
    c.fillStyle = '#2a0f16'
